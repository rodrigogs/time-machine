...
export class HistoryController {
    ...
    public showCurrent(editor: vscode.TextEditor) {
        ...
        if (document) {
            return this.internalOpen(
                this.findCurrent(document.fileName, this.getSettings(editor.document.uri)),
                editor.viewColumn
            )
        }
    }
    ...
    public findAllHistory(
        ...
        return new Promise<IHistoryFileProperties>((resolve, reject) => {
            if (!settings.enabled) {
    ...
    private internalSaveDocument(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (isOriginal) {
                ...
                // (Often the case...)
                const files = globSync(revisionPattern, { cwd: settings.historyPath.replace(/\\/g, '/') })
    ...
    private getHistoryFiles(patternFilePath: string, settings: IHistorySettings, noLimit?: boolean): Promise<string[]> {
        return new Promise((resolve, reject) => {
            ...
            glob(patternFilePath, { cwd: historyPath, absolute: true })
                .then((files: string[]) => {
